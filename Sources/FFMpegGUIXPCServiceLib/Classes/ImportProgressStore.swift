...
final class ConversionProgressStore: @unchecked Sendable {
    static let shared = ConversionProgressStore()

    private var values: [UUID: (progress: Double, done: Bool)] = [:]
    private let queue = DispatchQueue(label: "ConversionProgressStore")
